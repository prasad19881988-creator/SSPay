...
function ActionModal({ action, onClose, onComplete }) {
  ...
  const fields = () => {
    ...
    if (["Electricity","DTH / Broadband","All Bills"].includes(action)) return <><Field label="Consumer / Account Number" value={value.consumer||""} onChange={v=>set("consumer",v)}/><Field label="Amount" value={value.amount||""} onChange={v=>set("amount",v)} type="number" placeholder="0"/></>;
    if (action === "Freeze Card") return <div className="action-confirm"><ShieldCheck size={28}/><p>Card freeze request will be recorded in this demo.</p></div>;
    if (action === "Card Details") return <div className="action-confirm"><CreditCard size={28}/><p>Demo card details are already visible on the card screen.</p></div>;
    return <div className="action-confirm"><ShieldCheck size={28}/><p>{action} is ready. Continue to complete this demo flow.</p></div>;
  };
