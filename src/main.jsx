...
const ACTION_LABELS = {
  ...
  dth_broadband: "DTH / Broadband",
  all_bills: "All Bills",
  freeze_card: "Freeze Card",
  card_details: "Card Details",
};

function serviceActionId(text) {
  const map = {
    "Mobile Recharge": "mobile_recharge",
    "Electricity": "electricity",
    "DTH / Broadband": "dth_broadband",
    "All Bills": "all_bills",
  };
  return map[text] || String(text || "").trim().toLowerCase().replace(/\s+/g, "_");
}

function actionLabel(action) {
  return ACTION_LABELS[action] || String(action || "Payment");
}
...
function App() {
  ...
  const finishPendingAction = () => { const action = String(pendingAction || "").trim(); if (action) setModal("action"); else setModal(null); };
  ...
  const closeMpin = () => {
    ...
    // After creating MPIN, continue the exact action that opened the MPIN modal.
    // Do not clear pendingAction here; ActionModal needs it on the next render.
    if (action && mpinMode === "create") { setModal("action"); return; }
  ...
  return <div className="app">
    ...
    <main>
      {tab === "home" && <>
        ...
        <section className="quick-grid"><QuickAction icon={<ScanLine/>} text="Scan & Pay" onClick={() => requireMPIN("scan_pay")}/><QuickAction icon={<Send/>} text="Send Money" onClick={() => requireMPIN("send_money")}/><QuickAction icon={<ArrowDownToLine/>} text="Receive" onClick={() => requireMPIN("receive_money")}/><QuickAction icon={<IndianRupee/>} text="Bank Transfer" onClick={() => requireMPIN("bank_transfer")}/></section>
        <SectionHeader title="Pay & Services"/><section className="service-grid">{services.map(s => <Service key={s.text} icon={s.icon} text={s.text} onClick={() => requireMPIN(serviceActionId(s.text))}/>)}</section>
      ...
      {tab === "card" && <PageHeader title="SSPay Card" icon={<WalletCards/>} onBack={() => setTab("home")}><div className="bank-card big"><CardFace/></div><div className="security-box"><ShieldCheck size={24}/><div><strong>Secure by design</strong><p>Card details are masked in normal view.</p></div></div><div className="section-head"><h2>Card Controls</h2></div><section className="service-grid card-controls"><Service icon="🔒" text="Freeze Card" onClick={() => requireMPIN("freeze_card")}/><Service icon="👁️" text="Card Details" onClick={() => requireMPIN("card_details")}/><Service icon="⚙️" text="Card Settings" onClick={() => notify("Card settings opened")}/><Service icon="🧾" text="Card History" onClick={() => setTab("history")}/></section></PageHeader>}
    ...
    <nav className="bottom-nav"><NavButton active={tab === "home"} icon={<Home/>} text="Home" onClick={() => setTab("home")}/><NavButton active={tab === "offers"} icon={<Gift/>} text="Offers" onClick={() => setTab("offers")}/><button className="scan-main" type="button" onClick={() => requireMPIN("scan_pay")}><QrCode/></button><NavButton active={tab === "history"} icon={<History/>} text="History" onClick={() => setTab("history")}/><NavButton active={tab === "profile"} icon={<UserRound/>} text="Profile" onClick={() => setTab("profile")}/></nav>
...
function ActionModal({ action, onClose, onComplete }) {
  ...
  const canonical = actionLabel(action);
  ...
  const submit = (e) => {
    ...
    switch (action) {
      case "scan_pay":
        ...
        return onComplete(canonical, {
          title: `UPI Payment • ${value.upi}`,
          amount: `- ₹${Number(value.amount || 0).toFixed(2)}`,
          type: "debit",
          message: "Scan & Pay demo transaction completed"
        });
      ...
      case "send_money":
        ...
        return onComplete(canonical, {
          title: `Sent to ${value.upi}`,
          amount: `- ₹${Number(value.amount).toFixed(2)}`,
          type: "debit",
          message: "Send Money demo transaction completed"
        });

      case "receive_money":
        return onComplete(canonical, {
          title: "Receive Money • Request created",
          amount: "+ ₹0.00",
          type: "credit",
          message: "Receive request ready — share your UPI ID/QR with the payer"
        });
      ...
      case "bank_transfer":
        ...
        if (!value.amount || Number(value.amount) <= 0) return setError("Valid amount डालें");
        return onComplete(canonical, {
          title: `Bank Transfer • ${value.account.slice(-4)}`,
          amount: `- ₹${Number(value.amount).toFixed(2)}`,
          type: "debit",
          message: "Bank Transfer demo request submitted"
        });
      ...
      case "mobile_recharge":
        ...
        return onComplete(canonical, {
          title: `Mobile Recharge • ${value.mobile}`,
          amount: `- ₹${Number(value.amount).toFixed(2)}`,
          type: "debit",
          message: "Mobile recharge demo order created"
        });
      ...
      case "all_bills":
        ...
        return onComplete(canonical, {
          title: `${canonical} • ${value.consumer}`,
          amount: `- ₹${Number(value.amount).toFixed(2)}`,
          type: "debit",
          message: `${canonical} demo payment created`
        });

      case "freeze_card":
        return onComplete(canonical, { message: "Card freeze request completed in demo" });

      case "card_details":
        return onComplete(canonical, { message: "Card details unlocked for this demo session" });

      default:
        setError("This action is not configured. Please reopen the payment option.");
        return;
    }
  };
  ...
  const fields = () => {
    switch (action) {
      case "scan_pay":
        return <><Field label="UPI ID / QR reference" value={value.upi || ""} onChange={v => set("upi", v)} placeholder="name@upi"/><Field label="Amount" value={value.amount || ""} onChange={v => set("amount", v)} type="number" placeholder="0"/></>;
      case "send_money":
        return <><Field label="UPI ID / Mobile" value={value.upi || ""} onChange={v => set("upi", v)} placeholder="name@upi / 10 digit mobile"/><Field label="Amount" value={value.amount || ""} onChange={v => set("amount", v)} type="number" placeholder="0"/></>;
      case "receive_money":
        return <ReceivePanel/>;
      case "bank_transfer":
        return <><Field label="Account Number" value={value.account || ""} onChange={v => set("account", v.replace(/\D/g, ""))} inputMode="numeric"/><Field label="IFSC" value={value.ifsc || ""} onChange={v => set("ifsc", v.toUpperCase())} placeholder="SBIN0001234"/><Field label="Amount" value={value.amount || ""} onChange={v => set("amount", v)} type="number"/></>;
      case "mobile_recharge":
        return <><Field label="Mobile Number" value={value.mobile || ""} onChange={v => set("mobile", v.replace(/\D/g, ""))} inputMode="numeric"/><Field label="Operator" value={value.operator || ""} onChange={v => set("operator", v)} placeholder="Jio / Airtel / Vi / BSNL"/><Field label="Amount" value={value.amount || ""} onChange={v => set("amount", v)} type="number"/></>;
      case "electricity":
      case "dth_broadband":
      case "all_bills":
        return <><Field label="Consumer / Account Number" value={value.consumer || ""} onChange={v => set("consumer", v)}/><Field label="Amount" value={value.amount || ""} onChange={v => set("amount", v)} type="number" placeholder="0"/></>;
      case "freeze_card":
        return <div className="action-confirm"><ShieldCheck size={28}/><p>Card freeze request will be recorded in this demo.</p></div>;
      case "card_details":
        return <div className="action-confirm"><CreditCard size={28}/><p>Demo card details are already visible on the card screen.</p></div>;
      default:
        return <div className="action-confirm"><ShieldCheck size={28}/><p>Action configuration error.</p></div>;
    }
  };

  const instant = ["receive_money", "freeze_card", "card_details"].includes(action);

  return <div className="modal-backdrop">
    <form className="mpin-modal action-modal" onSubmit={submit}>
      <div className="modal-head">
        <div className="modal-icon"><ActionIcon action={canonical}/></div>
        <button type="button" className="modal-close" onClick={onClose}><X size={18}/></button>
      </div>
      <h2>{canonical}</h2>
      <p className="modal-subtitle">
        {instant
          ? "यह demo flow है। Real bank/UPI transaction के लिए authorized backend/provider connection जरूरी होगा।"
          : "Details भरें और Continue दबाएँ। यह अभी demo transaction flow है।"}
      </p>
      {fields()}
      {error && <div className="mpin-error">{error}</div>}
      <button className="primary-action" type="submit">
        {instant ? <><Check size={16}/> Continue</> : <><Check size={16}/> Submit</>}
      </button>
    </form>
  </div>;
}
