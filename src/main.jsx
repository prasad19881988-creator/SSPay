...
function ActionModal({ action, onClose, onComplete }) {
  ...
  const set = (key, val) => setValue(v => ({...v, [key]: val}));
  const submit = (e) => { e.preventDefault(); setError("");
    if (action === "Scan & Pay") { if (!value.upi) return setError("UPI ID / QR reference डालें"); if (!/^.{3,}$/.test(value.upi)) return setError("Valid UPI ID डालें"); return onComplete(action,{title:`UPI Payment • ${value.upi}`,amount:`- ₹${Number(value.amount||0).toFixed(2)}`,type:"debit",message:"Scan & Pay demo transaction completed"}); }
    if (action === "Send Money") { if (!value.upi) return setError("UPI ID या mobile number डालें"); if (!value.amount || Number(value.amount)<=0) return setError("Valid amount डालें"); return onComplete(action,{title:`Sent to ${value.upi}`,amount:`- ₹${Number(value.amount).toFixed(2)}`,type:"debit",message:"Send Money demo transaction completed"}); }
    if (action === "Receive Money") return onComplete(action,{title:"Receive Money • Request created",amount:"+ ₹0.00",type:"credit",message:"Receive request ready — share your UPI ID/QR with the payer"});
    if (action === "Bank Transfer") { if (!/^\d{9,18}$/.test(value.account||"")) return setError("Valid account number डालें"); if (!/^[A-Za-z]{4}0[A-Za-z0-9]{6}$/.test(value.ifsc||"")) return setError("Valid IFSC डालें"); if (!value.amount || Number(value.amount)<=0) return setError("Valid amount डालें"); return onComplete(action,{title:`Bank Transfer • ${value.account.slice(-4)}`,amount:`- ₹${Number(value.amount).toFixed(2)}`,type:"debit",message:"Bank Transfer demo request submitted"}); }
    if (action === "Mobile Recharge") { if (!/^\d{10}$/.test(value.mobile||"")) return setError("10 digit mobile number डालें"); if (!value.amount || Number(value.amount)<=0) return setError("Recharge amount डालें"); return onComplete(action,{title:`Mobile Recharge • ${value.mobile}`,amount:`- ₹${Number(value.amount).toFixed(2)}`,type:"debit",message:"Mobile recharge demo order created"}); }
    if (["Electricity","DTH / Broadband","All Bills"].includes(action)) { if (!value.consumer) return setError("Consumer / account number डालें"); if (!value.amount || Number(value.amount)<=0) return setError("Bill amount डालें"); return onComplete(action,{title:`${action} • ${value.consumer}`,amount:`- ₹${Number(value.amount).toFixed(2)}`,type:"debit",message:`${action} demo payment created`}); }
    if (action === "Freeze Card") return onComplete(action,{message:"Card freeze request completed in demo"});
    if (action === "Card Details") return onComplete(action,{message:"Card details unlocked for this demo session"});
    return onComplete(action,{message:`${action} completed`});
  };
  const fields = () => {
    if (action === "Scan & Pay") return <><Field label="UPI ID / QR reference" value={value.upi||""} onChange={v=>set("upi",v)} placeholder="name@upi"/><Field label="Amount" value={value.amount||""} onChange={v=>set("amount",v)} type="number" placeholder="0"/></>;
    if (action === "Send Money") return <><Field label="UPI ID / Mobile" value={value.upi||""} onChange={v=>set("upi",v)} placeholder="name@upi / 10 digit mobile"/><Field label="Amount" value={value.amount||""} onChange={v=>set("amount",v)} type="number" placeholder="0"/></>;
    if (action === "Receive Money") return <ReceivePanel/>;
    if (action === "Bank Transfer") return <><Field label="Account Number" value={value.account||""} onChange={v=>set("account",v.replace(/\D/g,""))} inputMode="numeric"/><Field label="IFSC" value={value.ifsc||""} onChange={v=>set("ifsc",v.toUpperCase())} placeholder="SBIN0001234"/><Field label="Amount" value={value.amount||""} onChange={v=>set("amount",v)} type="number"/></>;
    if (action === "Mobile Recharge") return <><Field label="Mobile Number" value={value.mobile||""} onChange={v=>set("mobile",v.replace(/\D/g,""))} inputMode="numeric"/><Field label="Operator" value={value.operator||""} onChange={v=>set("operator",v)} placeholder="Jio / Airtel / Vi / BSNL"/><Field label="Amount" value={value.amount||""} onChange={v=>set("amount",v)} type="number"/></>;
    if (["Electricity","DTH / Broadband","All Bills"].includes(action)) return <><Field label="Consumer / Account Number" value={value.consumer||""} onChange={v=>set("consumer",v)}/><Field label="Amount" value={value.amount||""} onChange={v=>set("amount",v)} type="number"/></>;
    return <div className="action-confirm"><ShieldCheck size={28}/><p>This demo action is now unlocked after MPIN verification.</p></div>;
  };
  const instant = ["Receive Money","Freeze Card","Card Details"].includes(action);
  return <div className="modal-backdrop"><form className="mpin-modal action-modal" onSubmit={submit}><div className="modal-head"><div className="modal-icon"><ActionIcon action={action}/></div><button type="button" className="modal-close" onClick={onClose}><X size={18}/></button></div><h2>{action}</h2><p className="modal-subtitle">{instant ? "यह demo flow है। Real bank/UPI transaction के लिए authorized backend/provider connection जरूरी होगा।" : "Details भरें और Continue दबाएँ। यह अभी demo transaction flow है।"}</p>{fields()}{error && <div className="mpin-error">{error}</div>}<button className="primary-action" type="submit">{instant ? <><Check size={16}/> Continue</> : <><Check size={16}/> Submit</>}</button></form></div>;
}
